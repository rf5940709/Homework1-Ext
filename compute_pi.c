...
double compute_pi_Gregory_avx(size_t dt)
{
    double pi = 0.0;
    register __m256d ymm0, ymm1, ymm2, ymm3, ymm4;
    ymm0 = _mm256_set1_pd(1.0);
    ...
    for (int i = 0; i <= dt - 4; i += 4) {
        ...
        ymm3 = _mm256_mul_pd(ymm3, ymm1);//x=i*2
        ymm3 = _mm256_add_pd(ymm0, ymm3);//x++
        ymm3 = _mm256_div_pd(ymm2, ymm3);//x=1/x or -1/x
        ymm4 = _mm256_add_pd(ymm4, ymm3);//pi=pi+x
    }
    double tmp[4] __attribute__((aligned(32)));
    _mm256_store_pd(tmp, ymm4);
...
int main(int argc, char *argv[])
{
    ...
    begin = clock();
    value = compute_pi_Gregory_avx(n * 1024*1024);
    end = clock();
    time = (double)(end - begin) / CLOCKS_PER_SEC;
    printf("compult_pi loop : %zu MB, vaile : %lf \n", n, value);
    printf("execution time of compute_pi_Gregory_avx()   : %lf sec\n", time);
